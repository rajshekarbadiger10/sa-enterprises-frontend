// app/page.tsx — fetches from Express backend
import Link from "next/link";
import { ArrowRight, Shield, Truck, HeadphonesIcon, Award, Gauge, Droplets, Thermometer, Layers, Settings, FlaskConical, ChevronRight, Zap } from "lucide-react";
...
async function api(path: string) {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api"}${path}`, { next: { revalidate: 60 } });
...
function normalize(p: any) {
  ...
  return {
    ...
    images: (p.images || []).map((img: any) => img.url?.startsWith("http") ? img.url : `${base}${img.url}`),
    tag: p.badge || undefined,
    specifications: Object.fromEntries((p.specifications || []).map((s: any) => [s.key, s.value])),
  };
}
...
export default async function HomePage() {
  const [catsRes, featRes, newRes] = await Promise.all([
    ...
    api("/products?isFeatured=true&badge=bestseller&limit=4"),
    api("/products?badge=new&limit=3"),
  ]);
  const categories = catsRes?.data?.categories || [];
  const featuredProducts = featRes?.data?.products || [];
  const newProducts = newRes?.data?.products || [];

  return (
    <div className="animate-page">
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: `radial-gradient(circle at 1px 1px, #ea580c 1px, transparent 0)`, backgroundSize: "32px 32px" }} />
        <div className="relative max-w-7xl mx-auto px-4 py-16 md:py-24">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-orange-600/20 border border-orange-600/30 text-orange-400 text-xs font-semibold px-3 py-1.5 rounded-full mb-6"><Zap size={12} />India's Trusted Industrial Products Supplier</div>
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">Precision Instruments for <span className="text-orange-400">Process Excellence</span></h1>
        ...
        <div className="max-w-7xl mx-auto px-4 py-5 grid grid-cols-2 md:grid-cols-4 gap-4">
          {trustBadges.map((b) => (
            <div key={b.title} className="flex items-center gap-3">
              <div className="w-10 h-10 bg-orange-100 text-orange-600 rounded-lg flex items-center justify-center shrink-0">{b.icon}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/search?q=" className="text-sm text-orange-600 hover:text-orange-700 font-medium flex items-center gap-1">All Categories <ChevronRight size={16} /></Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map((cat: any) => (
            <Link key={cat.slug} href={`/category/${cat.slug}`} className="card p-4 text-center hover:border-orange-200 group transition-all duration-200 hover:-translate-y-1">
              <div className="w-14 h-14 bg-orange-50 text-orange-600 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:bg-orange-100 transition-colors">{categoryIcons[cat.icon] || <Gauge size={28} />}</div>
        ...
        <section className="bg-orange-50/50 py-12">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/search?q=" className="text-sm text-orange-600 font-medium flex items-center gap-1">View All <ChevronRight size={16} /></Link>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {brands.map((brand) => (
            <Link key={brand} href={`/search?q=&brand=${encodeURIComponent(brand)}`}
              className="bg-white border border-slate-100 rounded-xl px-6 py-3 text-sm font-semibold text-slate-600 shadow-sm hover:border-orange-200 hover:text-orange-700 transition-all">{brand}</Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="bg-gradient-to-r from-orange-600 to-orange-500 rounded-2xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div><h2 className="font-display text-2xl md:text-3xl font-bold text-white mb-2">Need a Custom Solution?</h2><p className="text-orange-100">Our application engineers will help you select the right instrument.</p></div>
          <div className="flex gap-3 shrink-0">
            ...
            <a href="tel:+918431709221" className="inline-flex items-center gap-2 bg-white text-orange-600 font-semibold px-6 py-3 rounded-lg hover:bg-orange-50 transition-colors">Call Now</a>
          </div>
        </div>
      </section>
    </div>
